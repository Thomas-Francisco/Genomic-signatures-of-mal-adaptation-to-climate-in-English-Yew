...
This script calculates the residuals of the GEA model (later called genomic discrepancy index) and the genomic offset index using the RDA method.
The genomic discrepancy index is something we came up with to visualise how far the observed genomic compositions of populations are from what is predicted by the GEA model for the same climate. The idea is that populations will not have exactly the same genomic composition as predicted by the model's linear local adaptation relationship. If populations have more or less differences between predicted and observed genomic composition, this may indicate that their degree of local adaptation is lower than what is predicted as the optimal genomic composition, or that adaptation is not based on the same genetic basis. The reasons for this could be that the population has suffered from neutral events such as drift, which fixes potentially adaptive alleles, or that the populations are isolated, so that some adaptive loci will not be present in the population, or that popualtions are in adaptation lag or pre adapted to future climate. However, we must remember that other reasons, such as conditional neutrality or an incorrect or imprecise model, could cause these patterns.  
...
To calculate the genomic offset, we need to identify potentially climate-adaptive SNPs (or the entire set of SNPs, depending on the assumption of local adaptation) and then estimate the past/present (reference period) and future genomic composition of these SNPs. 
To estimate genomic composition, we will use genotype-environment association (GEA) methods on several sets of SNPs (all SNPs, outliers and random). The GEAs methods will identify the relationship between allelic variation and climatic variables. Then, based on this association and knowing the past/present and future climatic data, we can estimate the allelic composition of all locations knowing the climatic data -> we can interpolate/extrapolate the GEA relationship in space and time. This will estimate the genomic composition and allow us to perform a distance (euclidean in our study) to calculate the genomic offset. 
...
list_AGD <- c("AGD_index_all","AGD_index_random","AGD_index_random_same_AF","AGD_index_LC","AGD_index_MC","AGD_index_CG","AGD_index_random_AF_V2","AGD_index_all_outliers")
...
Adaptive_genomic_disruption <- function(RDA, K, observed_score, predicted_score, meta_data) {
  ...
  # Calculate the global AGD using Euclidean distance across all axes
...
for (x in 1:length(list_rda)) {
  ...
  # Calculate AGD and Relative Change using the updated function
  calculated_AGD <- Adaptive_genomic_disruption(rda, K, observed_values, predicted_values, meta_data_pop_order)
  
  # Create a dataframe for AGD and combined relative index values
  genomic_offset_df <- data.frame(
    Population = unlist(calculated_AGD$Population),
    Genomic_offset = unlist(calculated_AGD$Proj_offset_global),
    Relative_Index = unlist(calculated_AGD$Relative_change)  # Add the combined relative index
  )
  ...
  assign(paste0("AGD_index_bis_", set), genomic_offset_df)
}
```
...
plot <- ggplot(data = AGD_coord) + 
  ...
  geom_point(aes(x = Longitude, y = Latitude, fill = cut_interval(AGD_coord[,2], n = 5)), shape = 21,size=3, color = "black") +
...
```{r plot AGD scaled values 0-1, eval=FALSE, include=FALSE}
...
AGD_coord$scaled_values <- (AGD_coord[,2]-min(AGD_coord[,2]))/(max(AGD_coord[,2])-min(AGD_coord[,2]))

plot <- ggplot(data = AGD_coord) + 
  geom_sf(data = admin, fill = gray(0.92), size = 0) +
  geom_sf(data = shapefile, fill = "lightgrey", color = "black", size = 0.2) +  # Adding shapefile
  geom_point(aes(x = Longitude, y = Latitude, fill = cut_interval(AGD_coord[,5], n = 5)), shape = 21,size=3, color = "black") +
  #geom_point(aes(x = Longitude, y = Latitude, fill = cut(AGD_coord[,2], breaks=seq(global_min, global_max, length.out = 6), include.lowest = T)), shape = 21,size=3, color = "black") + #if we want to class with the same values the 4 models to compare color
  scale_fill_manual(
    values = colors,
...
plot <- ggplot(data = AGD_coord) + 
  ...
  geom_point(aes(x = Longitude, y = Latitude, fill = AGD_coord[, 2]), 
             shape = 21, size = 3, color = "black") +
  
  # Color scale: using a continuous gradient
  scale_fill_viridis(
    option = "C", # Choose a palette (e.g., "C" or "D")
    name = "Adaptive genomic disruption", 
