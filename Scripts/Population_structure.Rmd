...
In this script, we studied the population structure of *Taxus baccata* using the Adapcon-Gentree dataset, which contains 8252 SNPs, 452 trees and 4.37% of NAs.
We analysed the population structure using PCA and STRUCTURE software.

# Distribution map

## Common garden and range-wide populations

First, we can plot the populations in geographical space to gain insight into the geographical structure before looking at the genetic structure. We can plot the 29 populations for which we are calculating genomic indices, but also the 26 populations planted under common garden conditions used to evaluate the genomic offset predictions. 
...
First, we performed a PCA to study the population structure at both the individual and population scales. We also checked whether the structure of some individuals/populations is influenced by the percentage of missing data. To do this, we plotted the size of the sampled points on the PCA based on their missing data percentage of missing data, while the position of the points will reflect their genetic composition. We used the PCADAPT as function to perform the PCA analysis at the Individual level, and RDA at the population level (DA without covariables, where only PCA is executed; see the explanation of redundancy analysis in the script for candidate detection))   
...
Interpretation: we retained the two first axes as the most important for detecting global patterns of population structure. 
 ...
 Pca_genetic <- function(data, names, num_axes, meta_data) {
  ...
  for (i in 1:(num_axes - 1)) { #here, the first loop will select axis between 1 and numb_axis-1 so basically, if we take 3 axis, i will take values of 1 then 2, so PCx will ether be 1 or 2.
    for (j in (i + 1):num_axes) { #here, the second loop within the first one will take the values of j from i +1 to num axis so if num axis=3, we will have j ranging from 2 to 3 when i=1 and j=3 when i =2
      ...
      # genetic PCA
      ggplot_representation <- ggplot() +
        geom_point(data = PCa_df_imputed, aes(PCa_df_imputed[,2],PCa_df_imputed[,3], color = Country)) +
        scale_colour_manual(name = "Country",
                            values = c("orangered3", "gold2", "darkorchid3", "navyblue", "turquoise2", "green3", "blue", "red", "black", "gray", "orange", "darkgreen")) +
        xlab(paste0("PC", PCx, " ", "(", explained_variance$PCx, "%", ")")) + 
        ylab(paste0("PC", PCy, " ", "(", explained_variance$PCy, "%", ")"))
...
Finally, we wanted to calculate the PCA at the population level because we will use the population scores to account for population structure in GEA analyses. Since we're using the allelic frequencies, we cannot use the PCADAPT function because it cannot handle allelic frequencies. We  used the RDA function, which operates similarly as long as we do not include covariates. 

First, as before, we need to check the number of principal components (PCs) retained.
```{r Screeplot RDA}
...
#Save
pdf("C:/Users/tfrancisco/Documents/Thesis/Results/species/taxus/STRUCTURE/PCA/screeplot_PCA_Pop_level.pdf");screeplot(RDA_structure_genetic, type = "barplot", npcs=10, main="PCA Eigenvalues");dev.off()
...
Interpretation: we kept the two first axis.
...
Finally, we saved the table of population scores along the PCs axes to used them in variance partitioning and GEA analyses
...
Overall interpretation: The patterns observed are nearly identical to those found with PCA at the individual level. This is expected, but it allows us to verify if there were any mistakes during the calculation of allelic frequencies or in other processes.


# STRUCTURE software
      
The next step is to study the population structure using the STRUCTURE software.STRUCTURE is a Bayesian clustering approach that applies MCMC iterations. The MCMC begins by assessing randomly individuals to k groups (depending on the chosen k). Then, the allelic frequencies are calculated for each groups and the q-values at the individual scale and individuals are re-assigned depending on their Q-values. Each iteration of the MCMC estimates the allelic frequencies of the population and the Q-values of individuals, re-arranging groups of individuals to maximize the likelihood based on the Q-values. Initially, individuals are assigned randomly to the groups, so we apply a burning period to eliminate stochastic group assignments that occur before the MCMC begins to stabilize.
After the burning iteration period, each subsequent iteration is used to calculate the final Q-values for each individual, as these values will represent the mean Q-values for each iteration following the burning phase. We can also perform multiple runs for each K to assess whether the MCMC has converged to similar results across runs. 
...
Afterward, we used STRUCTURE Harvester in Linux to estimate the number of K values to retain (using the Evanno method and ln(K)) and to output the results in CLUMPP format. When running STRUCTURE, the names of the groups (1, 2, etc.) may not be consistent across runs for the same K. To reorganize the groups for estimating mean Q-values per individual across runs for the same K values, we used CLUMPP in Linux to reassign individuals to a consistent group name across runs.
Finally, we used the CLUMPAK web interface to visualize the delta k (Evanno method) and the ln(K) values (since STRUCTURE Harvester lacks a graphical option). We can also plot the traditional STRUCTURE graph showing the Q-values per individual. In this script, the goal is to plot the mean Q-value per population on a map to visualize the population structure.


First, we loaded the individual order from the STRUCTURE analysis to assign their Q scores.
...
Then, we calculated the mean Q-value per population and for each K, plotted it in a map. 
...
Interpretation: Based on the Best K from CLUMPAK and the biological relevance, we selected K=3 because it explains the largest pattern of population structure. Then, we will use k=3 for imputing genomic missing data within the three groups

Finally, we saved the scores for K=3 to used them as population structure proxy in variance partitioning models. 
```{r save k3 scores}
