...
This script enables to visualize and select the set of outliers loci to be used to compute genomic offset. To select candidates SNPs, we performed six GEA methods to detect SNPs potentially linked with climatic variables: RDA, pRDA, BAYPASS, LFMM and GF-raw, GF-corrected  
  
Each of these methods provides a set of candidate SNPs. To select our outliers, we will retain only those candidates identified by at least two methods and not in high linkage disequilibrium (LD). 
We performed the outlier detection using less conservative or more conservative set of thresholds:   
- less conservative: FDR < 10%, top 5%, and BF > 8 later called LC.    
...
we can represent in a venn diagram the sets to visualize the overlapping candidates (not shown here)
...
The UpSet graph is more practical because the Venn diagram can only plot up to five circles, while we have six sets. With the ggVennDiagram function, the results can become cluttered, making it difficult to interpret. The UpSet graph also allows us to represent only the overlaps, omitting the zero overlaps. This creates a simpler and more informative graph.
...
#save 
pdf(paste0("C:/Users/tfrancisco/Documents/Thesis/Results/species/taxus/GEA_new_var/outliers/figure/upset_plot_",threshold,"_set.pdf"));UpSetR::upset(snp_df, 
      sets = c("RDA", "pRDA", "LFMM", "BAYPASS", "GF_raw", "GF_corrected"),
      main.bar.color = "#45B6AA",
      matrix.color = "#D45176",
      keep.order = T);dev.off()
}
...
We can see that some of the outliers are located on the same contig. This can bias the signals by giving more weight to regions of the genome with a higher concentration of SNPs, which are in linkage disequilibrium (LD). This issue arises if we assume that the number of SNPs per contig is not size-dependent; thus, some contigs may be overrepresented due to stochastic events, potentially inflating their signals. To address this, we will remove SNPs that are in LD with other SNPs on the same contig. 
...
We can also extract a random set of SNPs to calculate genomic offset (GO) and compare them with the candidate markers. To create this random set, we need to randomly select the same number of SNPs as the candidates set (specifically, the less conservative one) from the full dataset of SNPs after removing all candidate SNPs identified by at least one GEA method.

Additionally, we might want to ensure that the selected random SNPs maintain the same proportion of different allelic frequencies as the outliers. This can be important because the proportion of allelic frequencies may explain the observed patterns (similar to GWAS). For example, if we have 44 outliers with 10 SNPs in allelic frequencies between 0 and 0.1, 22 between 0.5 and 0.6, and 12 between 0.9 and 1, we should select random SNPs while preserving this ratio of allelic frequencies. 

expliquer rapidement en disant que c'est pour LC

## Random with same allelic frequencies

New random selection including the same classes of allelic frequencies: 
...
Finally, we can calculate the mean allelic frequency of each populations for the neutral set of snps and calculate their class and identify randomly snps matching the class of allelic frequencies of the outliers. 
...
#random_set_taxus_not_overlap_both_dataset <- random_set_taxus_not_overlap_both_dataset

#save(random_set_taxus_not_overlap_both_dataset, file="C:/Users/tfrancisco/Documents/Thesis/Results/species/taxus/GEA_new_var/outliers/random_set_taxus_not_overlap_both_dataset.Rdata")

load("C:/Users/tfrancisco/Documents/Thesis/Results/species/taxus/GEA_new_var/outliers/random_set_taxus_not_overlap_both_dataset.Rdata")
...
SNP from the overlapping set ADAPCON-GENTREE and READAPT genomic markers

We can also create a set of random SNPs without considering the effect of allelic frequency. 
...
At the end, all the neutral selected SNPs are included in the common garden set of outliers and are not part of the candidates identified by at least one GEA method.
