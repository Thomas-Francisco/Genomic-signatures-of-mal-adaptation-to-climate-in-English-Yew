...
```{r GEA using gradient forest random 2 snps}
...
```{r save GEA GF random v2, include=FALSE}
...
```{r GEA using gradient forest all outliers snps}
...
```{r save GEA GF all outliers, include=FALSE}
...
for(i in 1:length(list_period_rmarkdown)){
  ...
  for(x in 1:(length(list_set_rmarkdown)-1)){
    ...
    #merge two dataset
GO_RDA_merge <- merge(GO_df <- get(paste0("GO_T_Adapcon_Gentree_GF_",set,"_",period)),GO_df <- get(paste0("GO_T_Adapcon_Gentree_GF_",set2,"_",period)),"Population")
#df
