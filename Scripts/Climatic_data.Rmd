...
The climatic values are very similar across climatic models, except for **BIO 9**, where the **UKESM1-0-LL model** predicts values that are very inconsistent with those predicted by the other models. This serves as a reminder that, in the absence of prior information suggesting one model over the others, we should be cautious when selecting future climatic values. Here, we will use the five climatic models as future values to calculate GO and then compare the predictions.
...
Now, we can load the folder with raster and create the raster stack with all the climatic variables for each climatic model
...
future_climatic_data <- data.frame(meta_data_pop$Population,raster::extract(future_climatic_data_raster, coords))#important to add raster:: because
#names of the raster (because they are not in the classic order Bio 1, 2 etc they are order by number so bio 1, 10, 11 etc)
...
# Comparison of the climatic envelope of *Taxus baccata*: European range vs. sampled population

We will compare the climatic envelope, using the 6 climatic vars retained for the past period, between the 29 populations and the occurrence point of Taxus baccata in Europe. 
...
We need to extract the coordinates from the SHP to extract the climatic data using the coordinates from the climatic data raster.  
```{r coord data}
...
### Combined of data set and scaling
...
One problem with the EUFORGEN map is that the presence data include planted populations, especially in central France, and also planted populations outside the distribution of Taxus, such as in the north of the UK. In order to evaluate more precise presence data, we will use the coordinates of the 29 genotypes for SNP populations + the 26 CG populations and the 242 populations sampled for SSR and compare them with the 29 populations.  
...
**What is below is a draft for future climatic data that might be useful**
