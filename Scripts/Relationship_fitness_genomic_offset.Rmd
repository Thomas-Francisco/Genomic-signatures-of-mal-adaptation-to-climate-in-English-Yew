...
for(i in 1:8) {  # Now we have 8 models (4 RDA + 4 GF)
  ...
  for(x in 1:length(list_traits)) {
    ...
    # Store the correlation in the dataframe
    lm_inf_df <- rbind(lm_inf_df, data.frame(Method=method,Model = model, Trait = trait,R2=round(r_squared,3),Intercept= round(Intercept,3),Coeff = round(coeff,3),Pvalue=round(pvalues,3)))
