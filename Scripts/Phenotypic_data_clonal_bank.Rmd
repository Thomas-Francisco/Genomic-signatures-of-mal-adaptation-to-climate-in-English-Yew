...
```{r mean value, message=FALSE, warning=FALSE}
#Growth
...
We also want to check the number of measured individuals per population, retaining only populations with more than two individuals
...
We can now run the MCMC GLMM models for each traits. We need to specify the priors as we are working with Bayesian models. We selected a non-informative prior due to the lack of prior information. Additionally, we set the number of iterations to 1,500,000, the burn-in period to 50,000, and the thinning interval to 500, sampling every 500 iteration. We ran each model four times per trait (4 MCMC chains) to ensure stability. All traits follow a Gaussian distribution with continuous data.
...
For growth traits, since trees in the clonal bank vary in age, we used stem length as a covariate to account for differences in shoot volume associated with older trees. 
...
To evaluate the performance of the models, we can investigate several aspects: 
